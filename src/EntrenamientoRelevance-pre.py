...
model = m.RelevanceClassifierSiamAtnMulLSTMPairShared()
ckpt = torch.load(os.path.join('..','trained','siam-atn', 'weights_relevanceClassifier_mul_lstm_shared_pre.pth'), map_location=device)
...
train(model, train_dl, val_dl, train_writer, val_writer, 
      epochs=5, trn_batches=1000, lr=1e-5)
